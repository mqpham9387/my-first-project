...
@Component({
  template: `
    <div class="row pl-4">
      ...
      <div>
        <a type="button" (click)="addQuotaionClick($event)">Add to Quot.</a>
        | &nbsp;
      </div>
      <div>
        <a type="button" (click)="bookClick($event)">Book</a>
      </div>
    </div>
    `
})
export class AirTariffActionRenderComponent implements ICellRendererAngularComp {
  params: any;
