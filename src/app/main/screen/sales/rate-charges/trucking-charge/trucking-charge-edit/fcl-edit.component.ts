...
@Component({
  selector: 'app-fcl-edit',
  templateUrl: './fcl-edit.component.html',
  styleUrls: ['./fcl-edit.component.css']
})
export class FclEditComponent implements OnInit {
  @Input() seaData;
  @Input() action;
  ...
  ngOnInit() {
    ...
    this.seaEditData = { ...this.seaData };
    this.containerType = this.seaData.containerType.unitID;
    this.seaEditData.containerType = this.seaData.containerType.unitID;
    this.seaEditData.validDateFrom = formatDateInput(this.seaData.validDateFrom);
    this.seaEditData.validDateTo = formatDateInput(this.seaData.validDateTo);

    switch (this.containerType) {
      case 'GP':
        this.fclForm.patchValue({ gpContainer: {...this.seaEditData }});
        this.ngbTitleTab = 'General Purpose Container';
        this.gpEdit = true;
        break;
      case 'RF':
        this.fclForm.patchValue({ rcContainer: {...this.seaEditData }});
        this.ngbTitleTab = 'Reefer Container';
        this.rcEdit = true;
        break;
      case 'OT':
        this.fclForm.patchValue({ otContainer: {...this.seaEditData }});
        this.ngbTitleTab = 'Open Top Container';
        this.otEdit = true;
        break;
      case 'FR':
        this.fclForm.patchValue({ frContainer: {...this.seaEditData }});
        this.ngbTitleTab = 'Flat Rack Container';
        this.frEdit = true;
        break;
      case 'ISOT':
        this.fclForm.patchValue({ isoContainer: {...this.seaEditData }});
        this.ngbTitleTab = 'ISO Tank';
        this.isoEdit = true;
        break;
    }
  }
    ...
    dialogResult.then(async (result) => {
      if (result === 'YES') {
        if (validData.isValid) {
          if (this.action === 'EDIT') {
            console.log(this.gpContClone);
            switch (this.containerType) {
              case 'GP':
                this.store.dispatch(new SeaFreightActions.UpdateSeaFreight({
                        index: +this.selectedIndex, newSeaFreight: this.gpContClone
                }));
                this.activeModal.close(this.gpContClone);
                MessageBox.show(
                  this.modal,
                  'Successfully saved!',
                  'Notification',
              ...
              case 'RF':
                this.store.dispatch(new SeaFreightActions.UpdateSeaFreight({
                        index: +this.selectedIndex, newSeaFreight: this.rcContClone
                }));
                this.activeModal.close(this.rcContClone);
                break;
              case 'OT':
                this.store.dispatch(new SeaFreightActions.UpdateSeaFreight({
                        index: +this.selectedIndex, newSeaFreight: this.otContClone
                }));
                this.activeModal.close(this.otContClone);
                break;
              case 'FR':
                this.store.dispatch(new SeaFreightActions.UpdateSeaFreight({
                        index: +this.selectedIndex, newSeaFreight: this.frContClone
                }));
                this.activeModal.close(this.frContClone);
                break;
              case 'ISOT':
                this.store.dispatch(new SeaFreightActions.UpdateSeaFreight({
                  index: +this.selectedIndex, newSeaFreight: this.isoContClone
                }));
                this.activeModal.close(this.isoContClone);
                break;
            }
          }
  ...
  protected setFCLBeforSave(): void {
    const gpContainer = this.fclForm.get('gpContainer') as FormGroup;
    const frContainer = this.fclForm.get('frContainer') as FormGroup;
    const otContainer = this.fclForm.get('otContainer') as FormGroup;
    const rcContainer = this.fclForm.get('rcContainer') as FormGroup;
