...
import { QuotationFilterModel } from 'src/app/main/model/search/filter-model';
...
@Component({
  selector: 'app-received-quotations',
  templateUrl: './received-quotations.component.html',
  styleUrls: ['./received-quotations.component.css'],
})

export class ReceivedQuotationsComponent implements OnInit, OnDestroy {
  @Input() data: any;
  @ViewChild('receivedQuotationGrid') receivedQuotationGrid: AgGridAngular;
  ...
  onGridReady(params) {
    ...
    const datasource = {
      getRows: (params: IGetRowsParams) => {
        ...
        this.store.dispatch(new QuotationActions.FetchReceivedQuotations(params));
        this.subscription = this.store
          ...
          .pipe(
            map(
              (quotationsState) => {
                return {
                  receivedQuotations: quotationsState.receivedQuotations,
                  totalReceivedQuotRows: quotationsState.totalReceivedQuotRows
                };
              }
            )
          )
          .subscribe((results: any) => {
            this.receivedQuotations = results.receivedQuotations;
            console.log(results);
            params.successCallback(results.receivedQuotations, results.totalReceivedQuotRows);
          });
      }
    }
  ...
  editClick(event?: any) {
    ...
    //         .subscribe((airFreights: Quotation[]) => {
    ...
    const datasource = {
      getRows: (params: IGetRowsParams) => {
        ...
        this.subscription = this.store
          .select('quotations')
          .pipe(
            map((receivedQuotationsState) => {
              const results = {
                receivedQuotations: receivedQuotationsState.receivedQuotations,
                totalReceivedQuotRows: receivedQuotationsState.totalReceivedQuotRows,
              };
              return results;
            })
          )
